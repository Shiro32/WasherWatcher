...
voice_stop_alarm	= "suibu'n keihouwo teisi si'masita."

voice_light_thirsty	= "no'doga kawa'itekimashita"
voices_heavy_thirsty = [
	"nodo'ga do'ndon kawa'itekimashita.",
	"no'doga gekiretuni kawa'iteimasu.",
...
def alert_moist_wetting()->None:
	...
	g.talk( voice_light_wet,  TALK_FORCE )
...
def alert_moist_wet()->None:
	...
	g.talk( voices_heavy_wet[rnd(len(voices_heavy_wet))], TALK_FORCE )
...
def check_moist_wet()->None:
	...
	if g.newest_moist < MOIST_THRESHOLD_WET -1 :
			...
			g.update_display_immediately()
			g.talk( "yatto kansou simasita.",  TALK_FORCE )
...
def alert_moist_drying()->None:
	...
	g.talk( voice_light_thirsty,  TALK_FORCE )
...
def alert_moist_dry()->None:
	...
	g.talk( voices_heavy_thirsty[rnd(len(voices_heavy_thirsty))], TALK_FORCE )
...
def check_soil_moist()->None:
	...
	if moist_status==MOIST_STATUS_NORMAL:			# 1.乾燥開始
		...
		# 乾燥を検出
		if g.newest_moist < MOIST_THRESHOLD_DRY:
			g.log( "MOIST", "DRYING-begin" )
			start_moist_status_drying()

		# 過剰湿潤を検出
		if g.newest_moist > MOIST_THRESHOLD_WET:
			g.log( "MOIST", "WETTING-begin" )
			start_moist_status_wetting()

	# 2.乾燥開始、3.乾燥継続
	elif moist_status==MOIST_STATUS_DRYING or moist_status==MOIST_STATUS_DRY:
		check_moist_dry()

	# 4.過剰湿潤開始、4.過剰湿潤継続
	elif moist_status==MOIST_STATUS_WETTING or moist_status==MOIST_STATUS_WET:
		check_moist_wet()
