...
# Mini Mac用のグローバル変数・グローバル関数用記述ファイル
...
# --------------------- 各種計測値 ---------------------
# # 最新の計測値（一応、10秒平均化したもの＝グラフの右端）
# グローバル化させたので、flower.pyで責任もって更新する
# いきなり各種判定に引っかからないように、常識的な値で開始
newest_moist	= 50
newest_temp 	= 20
newest_hum		= 50
newest_press	= 1000
...
def talk(message, force, wait=True):
	...
	if pi.read(SOUND_SW_PIN)==pigpio.HIGH : return
...
# ------------------------------------------------------------------------------
def power_save_mode()->None:
	"""一時的に最低電力消費モードにする
	・LiPo Charger/Boosterのヘボさ（瞬低しやがる）の対策として、一時的に省電力モードにする
	・本質的な対策ではないので、ちゃんとしたBoosterにするべき・・・。
	"""

	talk( "pawa- seibumo'-do ni hairimasu.", TALK_FORCE)
	talk( "ju'ubyouhodo si'zukani/si'terunode yu-esubi- kiri'kaenado site'kudasai.", TALK_FORCE )

	time.sleep(1)

	# 省電力
	os.system( "echo '1-1' |sudo tee /sys/bus/usb/drivers/usb/unbind" )	# USB
	os.system( "sudo /opt/vc/bin/tvservice -o" )	# HDMI
	os.system( "sudo ifconfig wlan0 down" )				# WiFi
	_turn_off_LED()																# フロントボタンライト
	epd.turnOffBackLight()												# OLED Backlight

	time.sleep(10)

	epd.turnOnBackLight()
	_turn_on_LED()
	os.system( "sudo ifconfig wlan0 up" )				# WiFi
	os.system( "sudo /opt/vc/bin/tvservice -p" )
	os.system( "echo '1-1' |sudo tee /sys/bus/usb/drivers/usb/bind" )

	talk( "bujini seikan de'kimasita. ho'xtu", TALK_FORCE )
