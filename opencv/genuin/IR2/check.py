...
#	picam2.configure(picam2.create_preview_configuration(main={"format": 'XRGB8888', "size": (1280, 960)    }))
	...
	w = img.shape[1]
	h = img.shape[0]
	img = img[int(h/3*2):int(h/3*3), int(w/3*1):int(w/3*2)] #top:bottom, left:right
...
corr, zoom = checkWasher( img, "light_off_template.png", 130)
...
corr, zoom = checkWasher( img, "light_2h_template.png", 130)
...
corr, zoom = checkWasher( img, "light_4h_template.png", 130)
