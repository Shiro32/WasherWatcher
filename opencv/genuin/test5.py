...
if( len(sys.argv)!=2 ):
	...
	#タイマーセットしていないチェック
	res, zoom = checkWasher( sys.argv[1], "off-dark_template.png", 100, 0.80 )
	if res:
		print(f"Timer is OFF")
		sys.exit()

	#タイマーセット済みのチェック
	res, zoom = checkWasher( sys.argv[1], "on4-dark_template.png", 100, 0.78 )
	if res:
		print(f"Timer(4h) is ON!")
		sys.exit()
	res, zoom = checkWasher( sys.argv[1], "on2-dark_template.png", 100, 0.78 )
	if res:
	...
	res,zoom = checkWasher( sys.argv[1], "off-dark_template.png", 200, 0.85 )
	if res:
		print( f"Door is open&off (zoom={zoom})" )
		sys.exit()

	res,zoom = checkWasher( sys.argv[1], "on4-dark_template.png", 200, 0.78 )
	if res:
		print( f"Door is open & on(4h) (zoom={zoom})" )
		sys.exit()

	res,zoom = checkWasher( sys.argv[1], "on2-dark_template.png", 200, 0.70 )
	if res:
